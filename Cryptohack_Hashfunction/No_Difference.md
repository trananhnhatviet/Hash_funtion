...
Nhìn khá là rắc rối nên là bài này mình thử bruteforce, vì muốn lấy tất cả các giá trị có 4 byte thì chỉ có 256^4 thì cũng hơn 4 tỉ tí thui

Ta sẽ code 1 hàm brute_force, ta sẽ lấy 1 dictionary để chứa các giá trị và hàm băm tương đương, nếu 2 giá trị khác nhau mà có cùng hàm băm thì return 
```
def brute_force():
    dic = {}
    while True:
        ran_byte = bytes(urandom(4))
        hash_result = hash(ran_byte)
        
        if hash_result in dic.values() and ran_byte not in dic.keys():
            other_entry = list(dic.keys())[list(dic.values()).index(hash_result)]
            return other_entry, ran_byte
        
        dic[ran_byte] = hash_result
```

Sau khi bruteforce thì sẽ thu được 2 giá trị a và b, chuyển về hex, connect với server để gửi a và b, và sẽ thu được flag thui
```
from os import *
from pwn import*
...
io.recvline('"The difference between treason and patriotism is only a matter of dates."\n')
to_send = '{"a": "' + m1 + '", "b": "' + m2+'"}'
print(to_send)
...
Flag của chall này là: **crypto{n0_d1ff_n0_pr0bl3m}**